...
class OidcServiceImpl(OidcTokenService):
    ...
    def validate_token_idp_for_signup(self, idp_token: str):
        ...
        try:
            # Validar JWT interno del broker
            print(" self.env_var.public_key :::::::::::::: ", self.env_var.public_key)
            claims = jwt.decode(idp_token, self.env_var.public_key, algorithms=Constants.ALGORITHM,
                                audience=self.env_var.idp_audience_signup_register)

            expected_scope = self.env_var.idp_scope_signup_token
    ...
    def generate_idp_token(self, internal_token: str) -> TokenIdpSchema:
        ...
        try:
            ...
            idp_token = wso2Client.get_access_token_for_signup(
                base_url=self.env_var.idp_token_url,
    ...
    def orchestrate_signup_user(self, data: SignupSubmitRequest) -> HttpResponseSchema:
        ...
        try:
            ...
            self.validate_token_idp_for_signup(idp_token.token)
            # (4) - INVOKE SIGNUP REGISTER

            response = signup_service.register_in_signup_core(
